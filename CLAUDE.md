...
- **Current best (honest) model:** `scripts/bbbp_v4.py` — fine-tuned ChemBERTa (`seyonec/ChemBERTa-zinc-base-v1`), same scaffold split as v2/v3. **0.912 AUROC**, beating tuned RandomForest by 0.051. Reasoning logged in `drug-discovery-wiki/decisions/2026-09-02-chemberta-fine-tune.md`.
...
- v2 and v3 pushed to GitHub (danimonz/bbb-penetration-model); v4 not yet pushed.
- **Environment:** Ubuntu. Main work in a `.venv` (Python 3.12), pinned in `requirements.txt` — now includes `torch` (CPU build) and `transformers` for v4. A **separate Python 3.11 env** is needed for PyTDC (3.12 breaks its install).
- **The ladder (improvement path):** RandomForest baseline → XGBoost + **scaffold split** (done) → tune (done) → fine-tune a chemical transformer (**done, currently here** — ChemBERTa 0.912 AUROC) → official **TDC leaderboard** entry. A graph neural net remains an optional side-branch, not required for the ladder to continue.
